package tech.hiphone.app.web.errors;
...
import tech.hiphone.app.config.AppConstants;
import tech.hiphone.commons.constants.ErrorCodeContants;
import tech.hiphone.commons.exceptioin.ServiceException;
import tech.hiphone.commons.web.errors.ErrorConstants;
import tech.hiphone.commons.web.errors.FieldErrorVM;
...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
    ...
    @ResponseBody
    public ResponseEntity<Problem> processServiceFailure(ServiceException ex, NativeWebRequest request) {
        Problem problem = Problem.builder().withType(ErrorConstants.SERVICE_ERROR_TYPE).withTitle("Service failed")
                .withDetail(ex.getMessage()).withStatus(defaultConstraintViolationStatus())
                .withCause(Optional.ofNullable(ex.getCause()).filter(cause -> isCausalChainsEnabled())
                        .map(this::toProblem).orElse(null))
                .with(CODE_KEY, ex.getCode()).build();
        return create(ex, problem, request);
    }
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    @ResponseBody
    public ResponseEntity<Problem> processNoSuchElementException(NoSuchElementException ex, NativeWebRequest request) {
        Problem problem = Problem.builder().withType(ErrorConstants.SERVICE_ERROR_TYPE).withTitle("Service failed")
                .withDetail(ex.getMessage()).withStatus(defaultConstraintViolationStatus())
                .withCause(Optional.ofNullable(ex.getCause()).filter(cause -> isCausalChainsEnabled())
                        .map(this::toProblem).orElse(null))
                .with(CODE_KEY, ErrorCodeContants.LACK_OF_DATA).build();
        return create(ex, problem, request);
    }
